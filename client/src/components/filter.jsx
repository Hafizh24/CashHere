...
export default function Filter({setProductData, setIsLoaded}){
    ...
    const handleSearch = async (keyword) =>{
        try{
            setIsLoaded(false)
            const response = await axios.get(`http://localhost:2000/products/get-product-filter?${keyword}`, {
                headers: {
                Authorization: `Bearer ${token}`,
                }
            });
    ...
    return(
        ...
        <Stack w={['70vw', '50vw']} direction={['column', 'row']}>                    
                ...
                <Select rounded={'full'} border={'1px'} onChange={(e) => {setOrderBy(e.target.value)}} value={orderBy} focusBorderColor='#3C6255' _hover={{}}>
                    <option value={'ByName'}>Sort by name</option>
                    <option value={'ByPrice'}>Sort by price</option>
                </Select>
                {orderBy === 'ByName' ? 
                <>
                <Select rounded={'full'} name="sort_alphabetical" value={formik.values.sort_alphabetical} onChange={(e) => {formik.setFieldValue("sort_alphabetical", e.target.value)}} defaultValue={''} focusBorderColor='#3C6255' _hover={{}} border={'1px'}>
                ...
                : 
                <>
                <Select rounded={'full'} name="sort_price" value={formik.values.sort_price} onChange={(e) => {formik.setFieldValue("sort_price", e.target.value)}} defaultValue={''} focusBorderColor='#3C6255' _hover={{}} border={'1px'}>
                    <option value='ASC'>Price: Low to High</option>
                    <option value='DESC'>Price: High to Low</option>
                </Select>
                </>}
                <Button rounded={'full'} type="submit" bgColor={'#3C6255'} _hover={{bg: '#61876E'}} color={'white'}> <Search2Icon/></Button>
        </Stack>
        </form>
    )
}