...
function AdminSettings(){
  ...
  const resetPassword = async () => {
    try{
      ...
      toast({
          title: "Success", description: `We've sent an email confirmation to reset your password`, status: "success", duration: 4000, position: "top"
      });
    }catch(err){
      console.log(err);
    }
  ...
  return(
      ...
      <Stack spacing={8} mx={'auto'} maxW={'lg'} minW={'32vw'} py={12} px={6}>
        <Stack align={'center'}>
          ...
          <Button onClick={() => {resetPassword()}} colorScheme='green'>Reset Password</Button>
        </Stack>
      </Stack>
      </>
  )
