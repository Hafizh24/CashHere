...
function AddProduct({getProducts}) {
    ...
    const handleSubmit = async (data) => {
        try{
            ...
            toast({
                title: "Success", description: `${data.get("name")} has been created`, status: "success", duration: 3000, position: "top",
            });
            getProducts()
            setLoading(false)
        }catch(err){
                    ...
                    <SimpleGrid columns={[1, null, 2]} spacing={10}>
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                <Input name='name' value={formik.values.name} onChange={formik.handleChange} type="text" focusBorderColor='#3C6255' _hover={{}} autoComplete='new' shadow={'md'}
                                error={formik.touched.name && Boolean(formik.errors.name)} placeholder="Insert product name"/>
                        ...
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                <Select defaultValue={category[0]?.id} name="category" _hover={{}} shadow={'md'} value={formik.setFieldValue.category} onChange={(e) => {
                                ...
                                {formik.touched.category && formik.errors.category ? (
                                    <Text mt={2} style={{color: 'red'}}>{formik.errors.category}</Text>
                                ) : null}
                            </FormControl>
                        </Box>
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                {formik.touched.price && formik.errors.price ? (
                                    <Text mt={2} style={{color: 'red'}}>{formik.errors.price}</Text>
                                ) : null}
                            </FormControl>
                        </Box>
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                <Input name="total_stock" value={formik.values.total_stock} onChange={formik.handleChange} shadow={'md'} type="number" autoComplete='new'
                                error={formik.touched.total_stock && Boolean(formik.errors.total_stock)}
                                placeholder="Insert product stock" _hover={{}}/>
                                {formik.touched.total_stock && formik.errors.total_stock ? (
                                    <Text mt={2} style={{color: 'red'}}>{formik.errors.total_stock}</Text>
                                ) : null}
                            </FormControl>
                        </Box>
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                <Input name="image" type="file" onChange={(e) => formik.setFieldValue("image", e.currentTarget.files[0])} _hover={{}} shadow={'md'} autoComplete='new'
                                error={formik.touched.image && Boolean(formik.errors.image)}/>
                                {formik.touched.image && formik.errors.image ? (
                                    <Text mt={2} style={{color: 'red'}}>{formik.errors.image}</Text>
                                ) : null}
                            </FormControl>
                        </Box>
                        <Box height='auto' p={7} rounded={'lg'} bgColor={'white'} shadow={"lg"}>
                            <FormControl>
                                ...
                                <Input name="description" value={formik.values.description} onChange={formik.handleChange} _hover={{}} shadow={'md'} type="text" autoComplete='new' placeholder="Insert product description"
                                error={formik.touched.description && Boolean(formik.errors.description)}/>
                                {formik.touched.description && formik.errors.description ? (
                                    <Text mt={2} style={{color: 'red'}}>{formik.errors.description}</Text>
                                ) : null}
                            </FormControl>
                        </Box>
                    </SimpleGrid>
